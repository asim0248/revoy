...
$keyword = (Request::input('keywords'))?Request::input('keywords'):'';
$keywordArray =array();
if($keyword!=''){
...
//echo '<pre>'; print_r($keywordArray); exit;
$where = '';
			  if($category_id!=''){
			 ...
			 if($bathrooms!=''){
			   ...
			   // $keywords = $keywords;
				//$keywordArray = array_filter(explode(',', trim($keywords))); // Split keywords into an array and remove empty values
				...
				$result = App\Model\Property::whereRaw("status = 'Yes'  AND admin_status = 'Yes' ".$where." ")
							...
							foreach ($keywordArray as $keyword) {
								$subQuery->orWhere('suburb', 'LIKE', '%' . $keyword . '%')
										// ->orWhere('street_address', 'LIKE', '%' . $keyword . '%')
										 ...
										 ->orWhere('full_contents', 'LIKE', '%' . $keyword . '%')
					...
					->when(!empty($keywordArray), function ($query) use ($keywordArray) {
        $query->orWhereHas('property_state', function ($stateQuery) use ($keywordArray) {
            $stateQuery->where('status', 'Yes')
                       ->where(function ($stateSubQuery) use ($keywordArray) {
                                ...
                                <div class="listing__page--wrapper">
                                    <div class="listing__header">
                                        <div class="listing__header--left">
                                            ...
                                            <p class="results__cout--text" id="">Showing <?=$result->total()?> Properties</p>
                                            <?php } ?>
                                        </div>
                                    ...
                                    <div class="listing__main--content">
                                        ...
                                        <div class="page__pagination--area">
                                           <?php echo $result->appends(request()->query())->render(); ?>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/revoycom/public_html/resources/views/search.blade.php ENDPATH**/ ?>