...
$podcast_category_dp = App\Model\Videocategory::whereRaw(" name = 'Podcasts' ")->select('id')->get()->toArray();

$video_cateogry_rs = array();
if(count($podcast_category_dp)>0){
$video_cateogry_rs = App\Model\Videocategory::whereRaw("status = 'Yes' AND pid = ".$podcast_category_dp[0]['id']."  ")->orderByRaw('id desc')->paginate(5);
...
?>
  ...
  <section class="blog__page--section podcast-posts-sec">
            <div class="container">
                <!-- row-1 -->
                <div class="row">
                    <div class="col-12">
                ...
                <?php if(count($video_cateogry_rs)>0){?>
                <?php foreach ($video_cateogry_rs as $k_cate=>$row_category){
					
					$result_videos  =  App\Model\Videos::whereRaw("status = 'Yes'  AND sub_category_id = ".$row_category['id']."  ")->orderByRaw('id DESC, id DESC')->take(6)->get();
					
					
					?>
                 <div class="row">
                     <h2 class="section__heading--title pb-4"><?=$row_category['name']?></h2>
                	<div class="col-xxl-8 col-xl-8 col-lg-8 col-md-12">
                        <div class="blog__page--wrapper">
                            <div class="row podcast-large">
                            <?php foreach ($result_videos as $k_v=>$row_video){?>
                            	<?php if($k_v<2){
                                ...
                                <div class="col-xxl-6 col-lg-6 col-md-6">
                                    <article class="blog__items">
                                        <div class="blog__thumbnail position-relative">
                                            ...
                                            <a href="<?=url('/')?>/podcats-detail/<?= $row_video['slug'] ?>-<?= $row_video['id'] ?>" class="video-popup__button"><i
                                            ...
                                            <ul class="blog__meta d-flex">
                                                <li>
                                                    <span class="blog__meta--icon">
                                                        ...
                                                        <span class="blog__meta--tag"><?=$row_video['tag_line']?></span>
                                                    </span>
                                                </li>
                                                <li class="blog__meta--list d-flex align-items-center">
                                                    ...
                                                    <span class="blog__meta--date"><?=date('d M Y',strtotime($row_video['created_at']))?></span>
                                                </li>
                                            </ul>
                                            <h3 class="blog__title"><a href="<?=url('/')?>/podcats-detail/<?= $row_video['slug'] ?>-<?= $row_video['id'] ?>"><?=$row_video['name']?></a></h3>
                                        </div>
                                    </article>
                                </div>
                                <?php } ?> 
                    ...
                    <?php if(count($result_videos)>2){?>
                    <div class="ol-xxl-4 col-xl-4 col-lg-4 col-md-12">
                ...
                <div class="row">
                    <div class="pagination-main">
                        <?php echo $video_cateogry_rs->render(); ?>
                    </div>
                </div>
            </div>
        </section>
...
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/revoycom/public_html/resources/views/blog_podcasts.blade.php ENDPATH**/ ?>