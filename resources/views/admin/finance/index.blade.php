    ...
    <div class="col-md-12">
        <div class="table-toolbar" style="margin-bottom: 8px;">
            <div class="btn-group pull-right">
                <a href="<?= URL::to('admin/finance/create') ?>"><button id="" type="button" class="btn green">
                ...
                <table class="table table-striped table-bordered table-hover" id="sample_1" >
                    ...
                    <tbody>
                        ...
                        foreach ($result_dp as $row) {
                            ...
                            if ($row['status'] == 'Yes') {
                                $status = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $row['id'] . ',\'finance/status\')" ><span  class="label label-success " data-title="Active"  ><i class="fa fa-check"></i> Active</span></a>';
                            } else {
                                $status = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $row['id'] . ',\'finance/status\')" ><span  class="label label-default" data-title="Inctive"  ><i class="fa fa-ban"></i> Inctive</span></a>';
                            }
                            ...
                            <tr class="odd gradeX">
                                ...
                                <td  >
                                    <?php if($row['image']!="") {?>
                                <img src="<?= url('/') . '/public/upload/finance/' . $row['image'] ?>" height="80" />
                                <?php } ?>
                                </td>
                                 <td  >
                                    <?= $row['sort_order'] ?>
                                ...
                                <td>
                                    <a href="<?= URL::to('/admin/finance/edit/' . md5($row['id'])) ?>"><button type="button" class="btn btn-primary btn-xs" data-title="Edit"  ><i class="fa fa-pencil"></i> Edit</button></a>
...
<script type="text/javascript">
    ...
    function delete_record(id) {
        bootbox.confirm('<div class="alert alert-danger alert-error" style="font-size:18px; margin-top: 20px;" >Are you sure you want to delete this ?</div>', function (result) {
            if (result) {
                window.location = "<?= URL::to('admin/finance/delete') ?>/" + id;
            }
        });
    }
