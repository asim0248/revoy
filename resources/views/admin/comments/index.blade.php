                ...
                <table class="table table-striped table-bordered table-hover" id="sample_1" >
                    ...
                    <tbody>
                        ...
                        foreach ($result_dp as $row) {
                            ...
                            if ($row['status'] == 'Yes') {
                                $status = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $row['id'] . ',\''.url('/').'/admin/comments/status\')" ><span  class="label label-success " data-title="Active"  ><i class="fa fa-check"></i> Active</span></a>';
                            } else {
                                $status = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $row['id'] . ',\''.url('/').'/admin/comments/status\')" ><span  class="label label-default" data-title="Inctive"  ><i class="fa fa-ban"></i> Inctive</span></a>';
                            }
                            ...
                            <tr class="odd gradeX">
                                ...
                                <td  >
                                    <?=App\Model\Posts::findDetail('heading',$row['post_id']) ?>
                                </td>
                                <td  >
                                    <?= $row['comment'] ?>
                                </td>
...
<script type="text/javascript">
    ...
    function delete_record(id) {
        bootbox.confirm('<div class="alert alert-danger alert-error" style="font-size:18px; margin-top: 20px;" >Are you sure you want to delete this ?</div>', function (result) {
            if (result) {
                window.location = "<?= URL::to('admin/comments/delete') ?>/" + id;
            }
        });
    }
