...
use Response;
use URL;
class ReviewsbrokerController extends Controller {
    /*
      |--------------------------------------------------------------------------
    ...
    public function index() {
		if(!Modules::permissions('reviewsbroker')){
			Redirect::to('admin/dashboard')->send();
		}
		
        return view('admin.reviewsbroker.index', ['title' => 'Broker Reviews']);
    }
	...
	public function listing(Request $request){
		...
		$data_provider = BrokerReviews::whereRaw(" 1=1  ".$sWhere." ")->skip($limit2)->take($limit1)->orderBy('id','DESC')->get();
		
	    $data_count = BrokerReviews::whereRaw(" 1=1  ".$sWhere." ")->count();
		...
		foreach ($data_provider as $row) {
				...
				if ($row->admin_status == 'Yes') {
                                $status = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $row->id . ',\''.url('/').'/admin/reviewsbroker/status\')" ><span  class="label label-success " data-title="Active"  ><i class="fa fa-check"></i> Active</span></a>';
                            } else {
                                $status = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $row->id . ',\''.url('/').'/admin/reviewsbroker/status\')" ><span  class="label label-default" data-title="Inctive"  ><i class="fa fa-ban"></i> Inctive</span></a>';
                            }
	...
	//----------------------------------------------------
		...
		public function actionGroupDelete($id){
		   BrokerReviews::whereRaw('id = ? ', array($id))->delete();
		   return true;
		}
    ...
    public function delete($id) {
        ...
        BrokerReviews::whereRaw('md5(id) = ? ', array($id))->delete();
        return Redirect::to('/admin/reviewsbroker');
    }
    ...
    public function view($id) {
        ...
        $data = BrokerReviews::whereRaw('md5(id) = ?  ', array($id))->get();
        return view('admin.reviewsbroker.view', ['title' => 'View Detail','row' => $data[0]]);
    }
    ...
    public function status(Request $request) {
        ...
        if ($request->_token) {
			 $model = BrokerReviews::find($request->id);
            if ($model->admin_status == 'Yes') {
                $set_status = 'No';
                $html = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $request->id . ',\''.url('/').'/admin/reviewsbroker/status\')" ><span  class="label label-default" data-title="Inctive"  ><i class="fa fa-ban"></i> Inctive</span></a>';
            } else {
                $set_status = 'Yes';
                $html = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $request->id . ',\''.url('/').'/admin/reviewsbroker/status\')" ><span  class="label label-success " data-title="Active"  ><i class="fa fa-check"></i> Active</span></a>';
            }
