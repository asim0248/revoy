...
class CustomersController extends Controller {
    ...
    public function index() {
		if(!Modules::permissions('customers')){
			Redirect::to('admin/dashboard')->send();
		}
		
		$result_dp = array(); //Agents::whereRaw('role_id=4' )->get()->toArray();
		
        return view('admin.customers.index', ['title' => 'Customers','result_dp'=>$result_dp]);
    }
	...
	public function listing(Request $request){
		...
		$data_provider = Agents::whereRaw(" 1=1 AND role_id=4  ".$sWhere." ")->skip($limit2)->take($limit1)->orderBy('id','DESC')->get()->toArray();
		
	    $data_count = Agents::whereRaw(" 1=1  AND role_id=4 ".$sWhere." ")->count();
		...
		foreach ($data_provider as $row) {
				...
				if ($row['status'] == 'Yes') {
                                $status = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $row['id'] . ',\''.url('/').'/admin/customers/status\')" ><span  class="label label-success " data-title="Active"  ><i class="fa fa-check"></i> Active</span></a>';
                            } else {
                                $status = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $row['id'] . ',\''.url('/').'/admin/customers/status\')" ><span  class="label label-default" data-title="Inctive"  ><i class="fa fa-ban"></i> Inctive</span></a>';
                            }
...
$actions .= '&nbsp; <a onclick="delete_record(\''.md5($row['id']).'\')"  href="javascript:void(0)" ><button type="button" class="btn btn-danger btn-xs" data-title="Delete" data-toggle="modal" data-target="#delete" ><i class="fa fa-trash"></i> </button></a>';
    ...
    public function status(Request $request) {
        ...
        if ($request->_token) {
            ...
            if ($model->status == 'Yes') {
                $set_status = 'No';
                $html = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $request->id . ',\''.url('/').'/admin/customers/status\')" ><span  class="label label-default" data-title="Inctive"  ><i class="fa fa-ban"></i> Inctive</span></a>';
            } else {
                $set_status = 'Yes';
                $html = '<a style="text-decoration:none;" href="javascript:void(0)" onclick="change_status(' . $request->id . ',\''.url('/').'/admin/customers/status\')" ><span  class="label label-success " data-title="Active"  ><i class="fa fa-check"></i> Active</span></a>';
            }
...
{
    $fileName = "customers_data_" . date('Ymd') . ".xls"; // Use .xls instead of .xlsx
    ...
    // Get data
    $result_dp = Agents::where('role_id', 4)->get();
