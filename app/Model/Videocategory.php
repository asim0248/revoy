...
class Videocategory extends Model
{
    ...
    public static function fillCombo($intValue = 0){
		$cmbo = '';
		$options = '';
        $cmbo .= '<select name="cmb_category" id="cmb_category" class="form-control">';
		$cmbo .='<option value="0">-- None-- </option>';
		$get_options = Videocategory::get_cat_selectlist(0,0);
       ...
       $get_options = DB::select( DB::raw("SELECT *			
		...
		if ($num_options > 0) {
			foreach ($get_options as $rows) { 
				...
				Videocategory::get_cat_selectlist($cat_id, $count);
			}
		}
